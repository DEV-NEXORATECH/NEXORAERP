@extends('layouts.erp', ['activePage' => 'sales-orders.create-page', 'pageTitle' => 'Tambah Sales Order'])
...
<section class="section">
    <div class="rounded-3xl border border-[#d7e3ef] bg-white p-6 sm:p-8 shadow-lg shadow-[#002F59]/5">
        <div class="mb-8 border-b border-[#d7e3ef] pb-5">
            <h2 class="mb-1">Tambah Sales Order</h2>
            <p class="muted">Buat order penjualan baru.</p>
        </div>
        <form method="post" action="{{ route('sales-orders.store') }}" class="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
            @csrf
            ...
            <div class="grid gap-1.5"><label>Order Date</label><input name="order_date" type="date" required></div>
            ...
            <div class="grid gap-1.5"><label>Proposal</label>
                <select name="proposal_id">
                    ...
                    @foreach($proposals as $proposal)
                        <option value="{{ $proposal->id }}">{{ $proposal->number ?? $proposal->title }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mt-4 flex items-center gap-3 md:col-span-2">
                <button class="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0059A7] px-5 py-2.5 text-sm font-black text-white shadow-sm transition hover:bg-[#002F59]">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 6L9 17l-5-5"/></svg> Simpan Order
                </button>
