@extends('layouts.erp', ['activePage' => 'procurement-contract-create', 'pageTitle' => 'Tambah Procurement Contract'])
...
<section class="section">
    <div class="rounded-3xl border border-[#d7e3ef] bg-white p-6 sm:p-8 shadow-lg shadow-[#002F59]/5">
        <div class="mb-8 border-b border-[#d7e3ef] pb-5">
            <h2 class="mb-1">Tambah Procurement Contract</h2>
            <p class="muted">Catat kontrak procurement dengan vendor.</p>
        </div>
        <form method="post" action="{{ route('procurement-contracts.store') }}" class="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
            @csrf
            ...
            <div class="grid gap-1.5"><label>Judul Kontrak</label><input name="title" required></div>
            <div class="grid gap-1.5"><label>Nilai Kontrak</label><input name="amount" type="number" min="0" required></div>
            <div class="grid gap-1.5"><label>Tanggal Mulai</label><input name="start_date" type="date"></div>
            <div class="grid gap-1.5"><label>Tanggal Selesai</label><input name="end_date" type="date"></div>
            <div class="grid gap-1.5"><label>Pengingat Perpanjangan</label><input name="renewal_reminder_date" type="date"></div>
