@extends('layouts.erp', ['activePage' => 'journal-entry-create', 'pageTitle' => 'Buat Journal Entry'])
...
<section class="section">
    <div class="rounded-3xl border border-[#d7e3ef] bg-white p-6 sm:p-8 shadow-lg shadow-[#002F59]/5">
        <div class="mb-8 border-b border-[#d7e3ef] pb-5">
            <h2 class="mb-1">Buat Journal Entry</h2>
            <p class="muted">Buat jurnal general ledger baru.</p>
        </div>
        <form method="post" action="{{ route('journal-entries.store') }}">
            @csrf
            <div class="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2 mb-6">
                <div class="grid gap-1.5"><label>Tanggal</label><input name="entry_date" type="date" value="{{ now()->toDateString() }}" required></div>
                <div class="grid gap-1.5"><label>Reference</label><input name="reference" placeholder="Manual / adjustment"></div>
                <div class="grid gap-1.5 md:col-span-2"><label>Memo</label><input name="memo" placeholder="Keterangan jurnal"></div>
            </div>
            <div class="mb-6">
                ...
                <div id="journal-lines">
                    @for($i = 0; $i < 2; $i++)
                        <div class="grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 p-3 mb-3 md:grid-cols-4">
                            ...
                            <input name="lines[{{ $i }}][debit]" type="number" min="0" value="{{ $i === 0 ? '' : 0 }}" step="0.01" placeholder="Debit">
                            <input name="lines[{{ $i }}][credit]" type="number" min="0" value="{{ $i === 1 ? '' : 0 }}" step="0.01" placeholder="Credit">
            ...
            <div class="mt-4 flex items-center gap-3">
                <button class="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0059A7] px-5 py-2.5 text-sm font-black text-white shadow-sm transition hover:bg-[#002F59]">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 6L9 17l-5-5"/></svg> Simpan Journal
                </button>
