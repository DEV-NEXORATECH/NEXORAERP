@extends('layouts.erp', ['activePage' => 'recurring-billing-create', 'pageTitle' => 'Tambah Recurring Billing'])
...
<section class="section">
    <div class="rounded-3xl border border-[#d7e3ef] bg-white p-6 sm:p-8 shadow-lg shadow-[#002F59]/5">
        <div class="mb-8 border-b border-[#d7e3ef] pb-5">
            <h2 class="mb-1">Tambah Recurring Billing</h2>
            <p class="muted">Buat tagihan berulang baru.</p>
        </div>
        <form method="post" action="{{ route('recurring-billings.store') }}" class="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
            @csrf
            <div class="grid gap-1.5"><label>Nama Subscription</label><input name="name" required></div>
            ...
            <div class="grid gap-1.5"><label>Frequency</label><select name="frequency"><option>monthly</option><option>weekly</option><option>quarterly</option><option>yearly</option></select></div>
            ...
            <div class="grid gap-1.5"><label>Next Invoice</label><input name="next_invoice_date" type="date" value="{{ now()->toDateString() }}" required></div>
            <div class="grid gap-1.5"><label>Status</label><select name="status"><option>active</option><option>paused</option><option>ended</option></select></div>
